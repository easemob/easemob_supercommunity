...
public class EMGroupManagerRepository extends BaseEMRepository{
        ...
        return new NetworkOnlyResource<String>() {
            ...
            protected void createCall(@NonNull ResultCallBack<LiveData<String>> callBack) {
                getGroupManager().asyncRemoveUserFromGroup(groupId, username, new EMCallBack() {
                    @Override
                    public void onSuccess() {
                        //移除某一个用户需要处理：删除社区记录： 退出社区下的群
                        groundRelation(groupId,username);
    ...
    //删除社区记录： 退出社区下的群
    private void groundRelation(String groupId,String username){
        GroundManager.getInstance().getGroundGroupByGroupId(groupId, new EMValueCallBack<GroundGroupBean>() {
            ...
            public void onSuccess(GroundGroupBean groundGroupBean) {
                //删除要踢出社区的记录
                GroundManager.getInstance().leaveGround(groundGroupBean.getGroundId(), username, new EMValueCallBack<Boolean>() {
                    @Override
                    public void onSuccess(Boolean value) {
                        // 踢出社区大厅外的其它群
                        GroundManager.getInstance().getGroundGroupBeanList(groundGroupBean.getGroundId(), new EMValueCallBack<List<GroundGroupBean>>() {
                            @Override
                            public void onSuccess(List<GroundGroupBean> value) {
                                for (GroundGroupBean groundGroupBean : value) {
                                    if (TextUtils.equals(groundGroupBean.getGroupId(),groupId)) {
                                        continue;
                                    }
                                    getGroupManager().asyncRemoveUserFromGroup(groundGroupBean.getGroupId(), username, new EMCallBack() {
                                        @Override
                                        public void onSuccess() { }
