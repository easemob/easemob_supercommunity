...
import com.community.easeim.imkit.adapter.EaseBaseAdapter;
...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private TextView mTvTimeBesideName;
    ...
    private void initView() {
        ...
        mTvTimeBesideName = (TextView) findViewById(R.id.tv_time_beside_name);
    ...
    protected void setLayoutStyle() {
        ...
        if (helper != null) {
            ...
            if (content != null) {
                ...
                if (itemStyle.getItemShowType() == 1){
                    content.setMaxWidth(DisplayUtil.dip2px(context,270f));
                }else {
                    content.setMaxWidth(DisplayUtil.dip2px(context,225f));
                }
            }
            if (usernickView != null) {
                usernickView.setTextColor(DemoApplication.getInstance().getColor(R.color.green_bg));
            }
        }
    }
    ...
    protected void setAvatarAndNick() {
        if (isSender()) {
            EaseUserUtils.setUserAvatar( EMClient.getInstance().getCurrentUser(), userAvatarView);
            //只要不是常规布局形式，就展示昵称
    ...
    private void showTimeBesideName() {
        if (mTvTimeBesideName != null) {
            mTvTimeBesideName.setVisibility(VISIBLE);
            mTvTimeBesideName.setText(EaseDateUtils.getTimestampString(getContext(), new Date(message.getMsgTime())));
        }
    }

    private void hideTimeBesideName() {
        if (mTvTimeBesideName != null) {
            mTvTimeBesideName.setVisibility(GONE);
        }
    }
